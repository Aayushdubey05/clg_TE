...
const Login = () => {
  ...
  const sendOTP = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/api/login/generate-otp",
        {
          method: "POST",
          headers: {
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      const getId = await fetch("http://localhost:8080/api/signup/getID", {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // Ensures JSON parsing
        },
        body: JSON.stringify({ stud_email: email }), // Properly stringify the body
      });

      const resp = await getId.json(); // Parse the JSON response
      console.log("respnnse",resp);

      toast.success("OTP verified successfully!");
      navigator(`/dashboard/${resp.id}`);
