    ...
    if (document_type === "admission_card") {
      ...
      fs.rename(req.file.path, filePath, (err) => {
        if (err) {
          return res
            .status(500)
            .json({ message: "Failed to save the file", error: err.message });
        }
        ...
        dashboard
          .update({ admission_documents: admissionDocs })
          .then(() => {
            res
              .status(200)
              .json({ message: "Admission card uploaded successfully" });
          })
          .catch((err) => {
            res
              .status(500)
      ...
      if (year && semester) {
        ...
        await dashboard.update({
          academic_records: dashboard.academic_records,
        });
      }
    }
...
exports.getStudentDashboard = async (req, res) => {
  try {
    ...
    const admissionYear = new Date(
      dashboard.signup.year_of_admission
    ).getFullYear();
    ...
    for (let i = 1; i <= yearsInCollege; i++) {
      responseData.academic_records[`year_${i}`] =
        dashboard.academic_records[`year_${i}`];
    }
